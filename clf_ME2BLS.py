...
class node_generator:
    ...
    def generator_nodes_feature(self, data, times, batchsize, nonlinear, clf_XAI, y):
        ...
        for idx in range(times):
            ...
            try:
                ...
                explained_value_weighted = np.sum(np.sum([array * weight for array, weight in zip(explained_value, y_ratio)], axis=0), axis=0)
                ...
                if non_zero_indices.shape[0] == 0:
                    ...
                    scaled_explained_value = -1 + 2 * (explained_value_weighted[non_zero_indices] - min_value) / (max_value - min_value)
        ...
        for i in range(1, len(self.Wlist)):
            nodes = np.column_stack((nodes, self.nonlinear(data[:, final_indices_list[i]].dot(self.Wlist[i]) + self.blist[i])))
    ...
    def transform_feature(self, testdata, final_indices_list):
        ...
        # testnodes = self.nonlinear(testdata.dot(self.Wlist[0]) + self.blist[0])
        for i in range(1, len(self.Wlist)):
            testnodes = np.column_stack((testnodes, self.nonlinear(testdata[:, final_indices_list[i]].dot(self.Wlist[i]) + self.blist[i])))
            # testnodes = np.column_stack((testnodes, self.nonlinear(testdata.dot(self.Wlist[i]) + self.blist[i])))
        return testnodes
...
class DGWBLS:
    ...
    def fit(self, oridata, orilabel, Lambda):
        ...
        mappingdata, self.final_indices_list = self.mapping_generator.generator_nodes_feature(data, self._Nf, self._N1, self._map_function, self.clf_XAI, label)
        # mappingdata = self.mapping_generator.generator_nodes_enhance(data, self._Nf, self._N1, self._map_function)
        enhancedata = self.enhance_generator.generator_nodes_enhance(mappingdata, self._Ne, self._N2, self._enhance_function)
        inputdata = np.column_stack((mappingdata, enhancedata))
    ...
    def partial_fit(self, extratraindata, extratrainlabel, extraLambda):
        ...
        self.P = self.P - (((self.P.dot(xdata.T)).dot(np.linalg.inv(np.linalg.inv(extraLambda) + temp))).dot(xdata)).dot(self.P)
...
class DGEBLS:
    ...
    def partial_fit(self, Xt, yt):
        ...
        for idx in range(len(self.bl_list)):
            ...
            bl_temp = copy.deepcopy(self.bl_list[idx])
            dl_temp = copy.deepcopy(self.dl_list[idx])
            ...
            for sp, lb in zip(Xt, yt):
                ...
                if bl.adwin_gamma.detected_change():
                    ...
                    Lamda = np.diag((self.predict(new_data) != new_label).astype(int))
                    if np.trace(Lamda) >= (self._n_class * self._max_data_pairs / 10):
                        new_bl.fit(new_data, new_label, Lamda)
                    else:
                        new_bl.fit(new_data, new_label, np.eye(new_data.shape[0]))

                    bl = copy.deepcopy(new_bl)

                if bl_temp.predict(sp_inv)[0] != lb:
                    bl_temp.adwin_gamma.add_element(0)
                else:
                    bl_temp.adwin_gamma.add_element(1)

                bl_temp.partial_fit(sp_inv, lb, np.eye(1))
                dl_temp.add_data(sp_inv, [lb])

            Acc_update_fit = sum(1 for pred, true_label in zip(bl.predict(Xt), yt) if pred == true_label) / yt.shape[0]
            Acc_update_partial_fit = sum(1 for pred, true_label in zip(bl_temp.predict(Xt), yt) if pred == true_label) / yt.shape[0]

            if Acc_update_fit >= Acc_update_partial_fit:
                ...
                self.bl_list[idx] = copy.deepcopy(bl_temp)
                self.dl_list[idx] = copy.deepcopy(dl_temp)
                self.Acc_list.append((Acc_update_partial_fit, self.bl_list[idx], self.dl_list[idx]))
    ...
    def update_alpha_list(self):
        ...
        for t in range(1, len(self.Acc_list) + 1):  # Range should start from 1
            ...
            numerator_p_minus_1 = sum(normalized_acc_difference_list[:t - 1])  # Subtract one from t for previous elements
